...
class MonthlyUnliquidatedCashAdvanceReportController extends Controller
{
    public function unliquidatedReport(MonthlyUnliquidatedCashAdvanceFilterRequest $filter)
    {
        ...
        try {
            $data = MonthlyUnliquidatedCashAdvanceService::unliquidatedCashAdvanceReport($generateReport->getDateFrom(), $generateReport->getDateTo());
    ...
    public function generateAsync(IncomeStatementFilterRequest $filter)
    {
        ...
        return new JsonResponse([
            ...
            'status' => 'queued',
            'cache_key' => $cacheKey,
            'polling_endpoint' => route('reports.unliquidated-cash-advance.status', ['cache_key' => $cacheKey]),
        ], 202);
    }
}
