...
use App\Http\Requests\Reports\BudgetReportFilterRequest;
...
{
    public function budgetReport(BudgetReportFilterRequest $filter)
    {
        ...
        try {
            $data = BudgetReportService::budgetReport($dateFrom, $dateTo);
            $payload = [
                'success' => true,
                'message' => 'Budget Report Successfully Retrieved.',
                'data' => $data,
    ...
    public function generateAsync(BudgetReportFilterRequest $filter)
    {
        $dateFrom = $filter->input('date_from');
        $dateTo = $filter->input('date_to');
