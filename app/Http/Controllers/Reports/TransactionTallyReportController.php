...
use App\Http\Requests\Reports\TransactionTallyReportRequestFilter;
use App\Jobs\GenerateReports;
use App\Services\Reports\TransactionTallyService;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Cache;
use Symfony\Component\HttpFoundation\JsonResponse;

class TransactionTallyReportController extends Controller
{
    public function transactionTally(TransactionTallyReportRequestFilter $filter)
    {
        $params = [
        ...
        $generateReport = new GenerateReports(ReportType::TRANSACTION_TALLY->value, $params);
        ...
        if ($isLargeReport) {
            ...
            return new JsonResponse([
                ...
                'polling_endpoint' => route(
                    'reports.transaction-tally.status',
                    ['cache_key' => $cacheKey]
                ),
            ], 202);
        }
        try {
            $reportData = TransactionTallyService::transactionTallyReport(
                $params['date_from'],
                $params['date_to']
            );
            $payload = [
    ...
    public function generateAsync(TransactionTallyReportRequestFilter $filter)
    {
        $params = [
            'date_from' => $filter->input('date_from'),
            'date_to' => $filter->input('date_to'),
        ...
        return new JsonResponse([
            ...
            'status' => 'queued',
            'cache_key' => $cacheKey,
            'polling_endpoint' => route('reports.transaction-tally.status', ['cache_key' => $cacheKey])
        ], 202);
    }
}
